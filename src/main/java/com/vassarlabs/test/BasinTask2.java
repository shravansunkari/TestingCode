...
public class BasinTask2 {
	...
	public void start() throws IOException, AppInitializationException, DSPException {
		...
		getData("/home/srikanth/Desktop/data/cds.csv", total_map, basinToSubBasinUUIDMap,
				subBasinToMicroBasinUUIDMap, 1);
		getData("/home/srikanth/Desktop/data/fp.csv", total_map, basinToSubBasinUUIDMap,
				subBasinToMicroBasinUUIDMap, 2);
		getData("/home/srikanth/Desktop/data/pt.csv", total_map, basinToSubBasinUUIDMap,
				subBasinToMicroBasinUUIDMap, 3);
		getData1("/home/srikanth/Desktop/micro_basin_level/MI_TANK.csv", total_map, basinToSubBasinUUIDMap,
				subBasinToMicroBasinUUIDMap, 4);
		getData("/home/srikanth/Desktop/data/others.csv", total_map, basinToSubBasinUUIDMap,
				subBasinToMicroBasinUUIDMap, 5);
		getData("/home/srikanth/Desktop/data/rwhs.csv", total_map, basinToSubBasinUUIDMap,
				subBasinToMicroBasinUUIDMap, 6);
	...
	public void getData(String csvFile, Map<Integer, basin_class> total_map,
			...
			int type) throws DSPException, AppInitializationException {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				try {
					...
					String micro_basin_code = row[20];
					if(micro_basin_code.length()==0){
						break;
					}
					int micro_basin_code1 = Integer.parseInt(row[20]);
					int micro_basin_count1 = Integer.parseInt(row[21]);
					double micro_basin_capacity1 = Double.parseDouble(row[22]);
					...
					String SubBasinName = getUUIDForSubBasinFromMicroBasinCode(micro_basin_code1, con,
							subBasinToMicroBasinUUIDMap);
					String BasinName = getUUIDForBasinFromSubBasinCode(SubBasinName, basinToSubBasinUUIDMap);
					String name = "Andhra Pradesh" + "##" + BasinName + "##" + SubBasinName + "##" + MicroBasinName;
					...
					if (type == 1) {
						...
						basinData.set_check_dam_proposed_count(micro_basin_count1);
						basinData.set_check_dam_proposed_capacity(micro_basin_capacity1);
					}

					total_map.put(micro_basin_code1, basinData);
					
					if(micro_basin_count1 == 0){
	...
	public String getBasinName(int sub_basin_code, Connection con, Map<String, List<String>> basinToSubBasinUUIDMap)
			throws DSPException {
	...
	public String getUUIDForSubBasinFromMicroBasinCode(int micro_basin_code, Connection con,
			Map<String, List<String>> subBasinToMicroBasinUUIDMap) throws DSPException {
	...
	public String getUUIDForBasinFromSubBasinCode(String name, Map<String, List<String>> basinToSubBasinUUIDMap)
			throws DSPException {
	...
	public int getSubBasinCode(int micro_basin_code, Map<Integer, HashSet<Integer>> subbasin_microbasin_map) {

		for (int code : subbasin_microbasin_map.keySet()) {
			HashSet<Integer> set = subbasin_microbasin_map.get(code);
			if (set.contains(micro_basin_code)) {
				return code;
			}
		}
		return -1;
